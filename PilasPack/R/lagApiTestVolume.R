...
#' @examples
#' lagApiTestVolume()
#' lagApiSubset(lab_file="quest", test_type="pcr") %>% lagApiTestVolume()
#'



lagApiTestVolume<-function(data_frame=All_Tests,
  ...
  for(i in 1:nrow(NewFrame)){
    NewFrame[i,2]<-nrow(subset(data_frame, timestamps.sampleCollectedDate==fechas[i]))
    NewFrame[i,3]<-nrow(subset(data_frame, timestamps.resultReportedDate==fechas[i]))
    NewFrame[i,4]<-nrow(subset(data_frame, timestamps.resultCreatedAtDate==fechas[i]))
  }
